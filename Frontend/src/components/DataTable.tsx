...
export function DataTable<T extends Record<string, unknown>>(
  ...
  const [sortDescriptor, setSortDescriptor] = useState<SortDescriptor>(
    { column: undefined, direction: "ascending" } as unknown as SortDescriptor
  );
  ...
  const sortedItems = useMemo(() => {
    ...
    const getValue = col?.sortValue ?? ((item: T) => getCellValue(item, col?.key ?? ""));
  ...
  return (
    <Table
      ...
      radius={radius}
    >
      <TableHeader columns={columns} className="text-default">
        {(column) => (
          <TableColumn
            key={column.key}
          ...
          >
            <span className={title({ size: "sm", fontWeight: "semibold", color: "default" })}>
              {column.label}
            </span>
          </TableColumn>
        )}
        ...
        {(item) => (
          <TableRow key={keyExtractor(item)}>
            {(columnKey) => {
              ...
              if (!column) return <TableCell>{getCellValue(item, String(columnKey))}</TableCell>;
              ...
              return (
                <TableCell align={cellAlign}>
                  <span className={title({ size: "sm", fontWeight: "normal", color: "default" })}>
                    {content}
                  </span>
                </TableCell>
              );
