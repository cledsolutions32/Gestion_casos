      ...
      renderCell: (user) => (
        <div
          className={`inline-flex items-center px-4 py-0.5 rounded-full text-sm font-medium capitalize ${
            user.rol === "admin" ? "bg-primary text-default" : "bg-gray text-black"
          }`}
        >
          <span className={title({ size: "sm", fontWeight: "medium", color: user.rol === "admin" ? "brown" : "default" })}>
            {user.rol}
          </span>
        </div>
      ),
    },
    {
      ...
      renderCell: (user) => (
        <div className="flex items-center justify-start gap-4">
          ...
          >
            <span className={title({ size: "sm", fontWeight: "bold", color: "blue", uppercase: true })}>
              editar
            </span>
          </Link>
          <Link
            ...
            className="cursor-pointer"
          >
            <span className={title({ size: "sm", fontWeight: "bold", color: "red", uppercase: true })}>
              eliminar
            </span>
          </Link>
        </div>
      ...
      <div className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-2">
            <div className="bg-primary rounded-full px-2">
              <span className={title({ size: "sm", fontWeight: "bold", color: "default" })}>
              {users.length}
              </span>
            </div>
        ...
        <ModalContent>
          ...
          <ModalBody>
            <p className={title({ size: "sm", fontWeight: "normal" })}>
              ¿Estás seguro de querer eliminar a <span className="font-bold">{userToDelete?.nombre || userToDelete?.email}</span>?
            </p>
          </ModalBody>
          <ModalFooter>
            <Button
